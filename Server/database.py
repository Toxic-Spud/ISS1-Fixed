...
class CustomDataBase:
    ...
    def remove_test_user(self):#remove this in production code only used for the test cases
        self._cursor.execute("delete from users where username = 'testUser'")
        self._connection.commit()
    ...
    def get_messages(self, userId:int):
        msgs = self._cursor.execute("select sender, message from messages where sender = ? or recipient = ?", (userId,)).fetchall()
        return(msgs)
    ...
    def create_employee(self, username):
        self._cursor.execute("""INSERT INTO Users(username,password) VALUES (?, ?)""")
        
    def revoke_users_com_key(self, username):
        pass
        

    def set_users_sign_key(self, username):
        pass

    def update_user_email(self, username):
        pass
...
def test_cases():
    AUTH_DATA_BASE.remove_test_user()
    try:
